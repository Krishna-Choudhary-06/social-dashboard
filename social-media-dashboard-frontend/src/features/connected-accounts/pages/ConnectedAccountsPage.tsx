...
export default function ConnectedAccountsPage() {
  ...
  const { list, connect, disconnect, refresh } = useConnectedAccounts();

  const displayAccounts = list.data || accounts;
  ...
  return (
    <Box>
            ...
            <Card component={motion.div} whileHover={{ y: -3 }} sx={{ height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  {a.status !== 'disconnected' ? (<><Button size="small" startIcon={<Refresh />} variant="outlined" fullWidth onClick={() => refresh.mutate(a.id)}>Refresh</Button><Button size="small" color="error" variant="outlined" onClick={() => setDisconnectId(a.id)}>Disconnect</Button></>) : (<Button size="small" startIcon={<Link />} variant="contained" fullWidth onClick={() => connect.mutate(a.platform)}>Connect</Button>)}
                </Box>
              </CardContent>
            </Card>
          </Grid>
      ...
      <Dialog open={!!disconnectId} onClose={() => setDisconnectId(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions><Button onClick={() => setDisconnectId(null)}>Cancel</Button><Button color="error" variant="contained" onClick={handleDisconnect} disabled={disconnect.isPending}>{disconnect.isPending ? 'Disconnecting...' : 'Disconnect'}</Button></DialogActions>
      </Dialog>
    </Box>
  );
}
