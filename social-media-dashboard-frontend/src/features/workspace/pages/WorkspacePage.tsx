  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid size={{ xs: 6, sm: 3 }}><StatCard label="Members" value="12" icon={<People sx={{ fontSize: 18 }} />} /></Grid>
        <Grid size={{ xs: 6, sm: 3 }}><StatCard label="Connected Accounts" value="5" icon={<AccountTree sx={{ fontSize: 18 }} />} /></Grid>
        <Grid size={{ xs: 6, sm: 3 }}><StatCard label="Reports Generated" value="48" change={12} trend="up" icon={<Assessment sx={{ fontSize: 18 }} />} /></Grid>
        <Grid size={{ xs: 6, sm: 3 }}><StatCard label="API Usage" value="67%" icon={<TrendingUp sx={{ fontSize: 18 }} />} /></Grid>
      </Grid>
            ...
            <List disablePadding>
              {recentMembers.map((m, i) => (
                <ListItem key={i} sx={{ px: 0, borderBottom: i < recentMembers.length - 1 ? '1px solid' : 'none', borderColor: 'divider' }}>
                  ...
                  <ListItemText 
                    primary={<Typography variant="body2" sx={{ fontWeight: 500 }}>{m.name}</Typography>} 
                    secondary={m.role} 
                  />
